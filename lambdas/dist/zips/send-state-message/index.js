...
function determineConculsionMet() {
  return new Promise((resolve, reject) => {
    console.log("into this section");
    raap
      .getToken()
      .then(response => {
        console.log("here is the raap response?", response);
        return raap.callReasoner({}, response.token);
      })
      .then(response => {
    ...
    if (event.state === 0 && event.channel_id) {
      ...
      if (payLoadSections.length === 1) {
        ...
        return slackApi
          ...
          .then(() => {
            if (
              !possibleStates[previousState].validateResponse(
                parsedResponseValue
              )
            ) {
